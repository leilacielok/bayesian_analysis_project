...
so that the intercept is included by default together with monthly and day-of-week indicators. Station-level heterogeneity is not included among the fixed effects, but is instead modeled through station-specific random intercepts.
...
Residual autocorrelation is illustrated in Figure \@ref(fig:fig-acf). While mild dependence persists for some parameters, it remains limited and does not compromise inference.
...
The temporal coefficients reported in Table \@ref(tab:posterior-effects) are now interpreted in detail. A strong seasonal pattern emerges, as relative to January, nearly all months exhibit negative coefficients, indicating substantially lower pollution levels throughout most of the year.
...
For compactness, Table \@ref(tab:posterior-effects) reports only the intercept and monthly effects. The omitted day-of-week coefficients are small and their 95\% HPD intervals include zero, suggesting no systematic weekly cycle once seasonal and station-level heterogeneity are accounted for.
