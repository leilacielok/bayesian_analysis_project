...
The Metropolis step for $\tau$ is implemented as a Gaussian random walk on the log scale. After tuning, a proposal standard deviation of 0.20 is adopted, yielding an acceptance rate of approximately 0.38. Posterior summaries and diagnostic plots are essentially unaffected by alternative choices, so this specification is retained for the final MCMC run.
...
The variance components show $z$-scores close to zero, equal to $-0.649$ for $\sigma^2$ and $-0.200$ for $\tau$, further supporting convergence of the global parameters.
