...
The Geweke diagnostic supports convergence, with all reported $z$-scores lying well within the conventional acceptance interval $[-2,2]$ (see Table \@ref(tab:tab-diagnostics)). For the global parameters, the values are $0.224$ for $\beta_1$, $-0.761$ for $\beta_2$, $-0.649$ for $\sigma^2$, and $-0.200$ for $\tau$. Representative station-level effects also yield $z$-scores close to zero (e.g., $0.111$, $-0.031$, and $0.063$ for $\alpha_1$, $\alpha_{58}$, and $\alpha_{116}$, respectively), providing no evidence against convergence.

Effective sample sizes indicate satisfactory overall efficiency, although mixing is not uniform across parameter blocks. In particular, $\beta_2$ and $\tau$ achieve ESS values close to the total number of retained draws (both around $960$), while $\sigma^2$ also shows a high ESS (approximately $743$). The ESS for $\beta_1$ is lower (about $178$), suggesting somewhat stronger autocorrelation for this parameter. The station-specific effects exhibit more heterogeneous ESS values, generally lower than those of the global parameters but still within an acceptable range (roughly between $65$ and $300$). Overall, these results indicate that the sampler mixes satisfactorily and provides a reliable effective amount of posterior information.

```{r tab-diagnostics, echo=FALSE, warning=FALSE}
library(knitr)

diagnostics_table <- data.frame(
  Parameter = c("$\\beta_1$", "$\\beta_2$", "$\\sigma^2$", "$\\tau$"),
  `Geweke z-score` = c(0.224, -0.761, -0.649, -0.200),
  ESS = c(178.4, 960.0, 742.995, 960.0)
)
...
kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Convergence diagnostics and effective sample sizes for key parameters",
  escape = FALSE,
  table.envir = "table",
  position = "H"
)
...
if (knitr::is_latex_output()) {
  cat("\\begin{table}[H]\n")
  cat("\\centering\n")
  cat("\\caption{Posterior means and approximate percentage effects relative to the baseline (January, Monday).\\label{tab:posterior-effects}}\n")
  cat(knitr::kable(tab, format = "latex", booktabs = TRUE), sep = "\n")
  cat("\\end{table}\n")
} else {
  knitr::kable(
    tab,
    caption = "Posterior means and approximate percentage effects relative to the baseline (January, Monday)."
  )
}
```
...
We now turn to the interpretation of the temporal coefficients.
...
The magnitude of these effects is economically large: spring, summer, and early autumn months are associated with reductions often exceeding 30–50%. December appears statistically indistinguishable from January, suggesting a return to high pollution levels during winter.
...
A one-at-a-time sensitivity analysis is conducted to assess the robustness of posterior inference to alternative prior specifications. Starting from the baseline configuration $c_{\text{prior}}=100$, $A_\tau=5$, and $b_\sigma=0.5$, each hyperparameter is varied separately while keeping the others fixed.

Three dimensions of prior calibration are considered. First, the prior variance of the regression coefficients is modified by setting $c_{\text{prior}}=10$ and $c_{\text{prior}}=1000$, in place of the baseline value $100$. Second, the scale parameter of the half-Cauchy prior on the group-level standard deviation is varied from $A_\tau=5$ to $A_\tau=2.5$ and $A_\tau=10$. Third, the scale parameter of the inverse-gamma prior on the observation variance is increased from $b_\sigma=0.5$ to $b_\sigma=1$, while keeping $a_\sigma=2$ fixed.

Posterior estimates are remarkably stable across all specifications. In particular, the posterior mean of the intercept, as well as those of $\sigma^2$ and $\tau$, remain virtually unchanged under all alternative priors considered. No meaningful differences emerge in the substantive interpretation of the temporal effects or in the magnitude of the station-level heterogeneity.

These findings indicate that posterior inference is not materially driven by the specific prior calibration adopted in the baseline model. Instead, the likelihood appears to dominate the posterior distribution, suggesting that the data are sufficiently informative for the parameters of interest.

From a computational perspective, acceptance rates for the Metropolis step on $\tau$ remain within a satisfactory range across specifications, indicating that the sampler retains stable performance under moderate prior perturbations.

Table \@ref(tab:sensitivity) reports the posterior means of the main parameters under the alternative prior configurations.
...
tab_sensitivity <- data.frame(
  Scenario = gsub("_", " ", sensitivity_results$scenario),  # evita errore LaTeX
...
Overall, the results provide reassuring evidence that the main conclusions are robust to reasonable variations in prior assumptions.
